...
public class ComputerDAO
{
	...
	public static String FindComputerName = "SELECT  computer.name as computer_name, computer.id as computer_id, computer.introduced, computer.discontinued, computer.company_id, company.name as company_name FROM computer LEFT JOIN company on company.id=computer.company_id WHERE LOWER(computer.name) LIKE :recherche OR LOWER(company.name) LIKE :recherche OR introduced LIKE :recherche OR discontinued LIKE :recherche;";
	...
	public List<Computer> allComputer() {
		return (List)namedParameterJdbcTemplate.query(Select,new ComputerMapper());
	}
	...
	public List<Computer> pageComputer(int tri, String column, int limit, int offset) {
		...
		return (List)namedParameterJdbcTemplate.query(requete, namedParameters, new ComputerMapper());
	}
	...
	public List<Computer> findComputerName (String recherche) {
		recherche=recherche.toLowerCase();
		recherche="%"+recherche+"%";
		SqlParameterSource namedParameters  = new MapSqlParameterSource().addValue("recherche",recherche);
		return (List)namedParameterJdbcTemplate.query(FindComputerName, namedParameters, new ComputerMapper());
	}
